 ...
 */
function findExistingBinary(basePath) {
 ...
 */
function buildTerminalCommand(binaryPath, args) {
    const isWin = process.platform === 'win32';
    if (isWin) {
...
function activate(context) {
    ...
    if (!serverPath) {
        ...
        vscode.window.showErrorMessage(`Fennec LSP binary not found. Please ensure it is built in 'build/' or set 'fennec.lsp.path' in settings.`);
    }
    else {
